...
jint naSetup(JNIEnv *pEnv, jobject pObj, int pWidth, int pHeight) {
	...
	// of AVPicture
	ret = avpicture_fill((AVPicture *)frameRGBA, buffer, AV_PIX_FMT_RGBA,
			pWidth, pHeight);
			
	if (ret < 0) {
			LOGI("avpicture_fill() failed");
	}else{
			LOGI("avpicture_fill()  size:%d", ret);
	}
		...
		if(packet.stream_index==videoStream) {
			...
			if(frameFinished) {
				...
				if (ANativeWindow_lock(window, &windowBuffer, NULL) < 0) {
					...
					LOGI("windowBuffe.stride:%d,  frameRGBA->linesize[0]:%d",  windowBuffer.stride,  frameRGBA->linesize[0]);
